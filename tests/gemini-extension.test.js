...
// always-on context, commands/*.toml for /rig + /rig-review, and
...
// carries the load-bearing rules — i.e. if the adapter stops wiring rig.
...
const REUSED_SKILLS = ['skills/rig/SKILL.md'];
// Gemini CLI auto-loads this exact path for extension hooks. Rig's
...
test('manifest names the rig extension with a pinned version', () => {
...
test('contextFileName resolves to a file carrying the rig rules', () => {
  const manifest = loadManifest();
