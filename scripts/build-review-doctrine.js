#!/usr/bin/env node
// Regenerates the ladder block inside skills/ponytail-review/SKILL.md from the
// canonical "## The ladder" section of skills/ponytail/SKILL.md, so the review
// skill's copy of the ladder can never drift from the one ponytail actually
...
const SOURCE_SKILL = path.join(ROOT, 'skills', 'ponytail', 'SKILL.md');
const TARGET_SKILL = path.join(ROOT, 'skills', 'ponytail-review', 'SKILL.md');
...
// Pull "## The ladder" verbatim out of skills/ponytail/SKILL.md: from that
...
function deriveLadder(sourceSkillText) {
  ...
  if (!heading) throw new Error('skills/ponytail/SKILL.md has no "## The ladder" section');
...
function render(sourceSkillText, targetSkillText) {
  ...
  if (!markerRe.test(targetSkillText)) {
    throw new Error(`skills/ponytail-review/SKILL.md is missing the ${BEGIN_MARKER} / ${END_MARKER} markers`);
  }
