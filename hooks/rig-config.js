...
//   1. RIG_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/rig/config.json (any platform, if set)
//      - ~/.config/rig/config.json (macOS / Linux fallback)
//      - %APPDATA%\rig\config.json (Windows fallback)
//   3. 'full'
...
// "stop rig" / "normal mode" turn rig off, but only as a standalone
...
// rig: only embed the plugin install path in a statusline shell command when
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'rig');
  }
...
function getClaudeDir() {
  // rig: CLAUDE_CONFIG_DIR overrides ~/.claude, matching Claude Code.
